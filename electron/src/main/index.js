...
// ---------------------------------------------------------------------------
// Update System (auto-updates via GitHub releases)
// ---------------------------------------------------------------------------
      ...
      mod.get(currentUrl, { headers: { 'User-Agent': 'bakers-raid-monitor' } }, (res) => {
        ...
        res.on('data', chunk => {
          ...
          if (total > 0 && win && !win.isDestroyed())
            win.webContents.send('update-progress', { status: 'downloading', percent: Math.round(received / total * 100) })
        })
        res.pipe(file)
...
async function downloadAndReplace(url) {
  ...
  if (win && !win.isDestroyed()) win.webContents.send('update-progress', { status: 'downloading', percent: 0 })

  try {
    await downloadFile(url, tempPath)
    if (win && !win.isDestroyed()) win.webContents.send('update-progress', { status: 'ready' })
...
// ---------------------------------------------------------------------------
app.whenReady().then(() => {
  // Verificar y aplicar actualización pendiente antes de crear ventana
  ...
  // Auto-check para updates 3s después (silent)
